...
void main() {
  ...
  final sprintPlanningApiProvider = SprintPlanningApiProvider();

  test('Utils Call Test', () {
    final util = sprintPlanning.util;
    util.callUtil();
    expect(util.call, "call");
  });

  test('API Provider Test', () async {
    sprintPlanningApiProvider.client = MockClient((request) async {
      final mapJSON = {
        "ceremonies": [
          {
            "id": 1,
    ...
    final ceremonyItemResult = sprintPlanningApiProvider.getCeremonyItemModel();
